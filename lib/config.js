...
(function (root, factory) {
  ...
  const DEFAULT_SETTINGS = {
    ...
    source: 'cbr',         // cbr (ЦБ РФ) | market (рыночный)
    markup: 0,             // наценка в процентах, чтобы учесть комиссию
    minAmountRub: 0,       // не показывать конверсию дешевле этой суммы
    manualRates: {},       // курсы, заданные руками: { USD: 95, EUR: 105 }
  ...
  function rateFor(code, rates, settings) {
    const manual = (settings && settings.manualRates) || {};
    if (manual[code] > 0) return manual[code];
    ...
    if (settings && settings.scaleOthers) {
      const factor = anchorFactor(rates, manual);
  ...
  function toTarget(amount, code, targetCode, rates, settings) {
    ...
    const markup = (settings && settings.markup) || 0;
    const value = (amount * rateSrc) / rateDst;
    return markup ? value * (1 + markup / 100) : value;
  }
